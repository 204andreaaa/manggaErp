...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="card-body py-3 d-flex align-items-center justify-content-between flex-wrap gap-2">
      ...
      <div class="d-flex align-items-center gap-2">
        <div class="input-group input-group-sm">
          ...
          <input type="text" id="dtSearch" class="form-control form-control-sm" placeholder="Search Product Family...">
        </div>
        @if(auth()->user()->hasPermission('products.create'))
          <button class="btn btn-primary btn-sm px-3" data-bs-toggle="modal" data-bs-target="#modalCreate">
            <i class="bx bx-plus me-1"></i> Add Product Family
          </button>
        @endif
      </div>
    </div>
  ...
  <div class="card">
    <div class="table-responsive">
      <table id="dtFamilies" class="table table-hover align-middle mb-0 w-100">
        <thead class="table-light">
          <tr>
            <th style="width:8%;" class="text-uppercase fw-bold small">NO</th>
            <th class="text-uppercase fw-bold small">FAMILY NAME</th>
...
<div class="modal fade" id="modalCreate" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <form id="formCreate" class="modal-content" method="POST" action="{{ route('erp.product-families.store') }}">
      @csrf
      <div class="modal-header border-bottom">
        <h5 class="modal-title fw-bold"><i class="bx bx-plus-circle me-2 text-primary"></i>Add Product Family</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <div class="modal-body">
        <div class="mb-3">
          <label class="form-label fw-semibold">Family Name <span class="text-danger">*</span></label>
          <input name="family_name" class="form-control" required placeholder="e.g. Electronics, Raw Material">
        </div>
        <div class="mb-3">
      ...
      <div class="modal-footer border-top">
        ...
        <button class="btn btn-primary" type="submit" id="btnCreateFamily"><i class="bx bx-save me-1"></i>Save</button>
      </div>
    </form>
  </div>
</div>
...
<div class="modal fade" id="modalEdit" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <form id="formEdit" class="modal-content" method="POST">
      ...
      <div class="modal-header border-bottom">
        <h5 class="modal-title fw-bold"><i class="bx bx-edit-alt me-2 text-warning"></i>Edit Product Family</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <div class="modal-body">
        <div class="mb-3">
          <label class="form-label fw-semibold">Family Name <span class="text-danger">*</span></label>
          <input name="family_name" id="edit_family_name" class="form-control" required>
        </div>
        <div class="mb-3">
          <label class="form-label fw-semibold">Description</label>
      ...
      <div class="modal-footer border-top">
        <button class="btn btn-outline-secondary" data-bs-dismiss="modal" type="button">Cancel</button>
        <button class="btn btn-warning text-white" type="submit" id="btnEditFamily"><i class="bx bx-save me-1"></i>Update</button>
      </div>
    </form>
  </div>
</div>
...
$(function () {
    ...
    const DT = $('#dtFamilies').DataTable({
        processing: true,
        serverSide: true,
        searching: false,
        ajax: {
            url: "{{ route('erp.product-families.datatable') }}",
            type: "POST",
            headers: { 'X-CSRF-TOKEN': CSRF }
        },
        columns: [
            { data: 'rownum',      name: 'id',          className: 'align-middle fw-semibold', orderable: false },
            { data: 'family_name', name: 'family_name', className: 'align-middle fw-bold' },
    ...
    $('#formCreate').on('submit', async function (e) {
        ...
        try {
            ...
            toast('Product Family created successfully!');
        } catch (err) {
    ...
    window.openEdit = function (id, name, desc) {
        $('#edit_id').val(id);
        $('#edit_family_name').val(name);
        $('#edit_description').val(desc || '');
        $('#formEdit').attr('action', "{{ route('erp.product-families.update', ':id') }}".replace(':id', id));
        new bootstrap.Modal($('#modalEdit')[0]).show();
    };

    $('#formEdit').on('submit', async function (e) {
        e.preventDefault();
        const btn = $('#btnEditFamily');
        btn.prop('disabled', true).html('<span class="spinner-border spinner-border-sm me-1"></span>Saving...');
        try {
            const res = await fetch(this.action, { method: 'POST', body: new FormData(this), headers: { 'X-CSRF-TOKEN': CSRF } });
            const json = await res.json();
            ...
            DT.ajax.reload(null, false);
            toast('Product Family updated successfully!');
        } catch (err) {
            Swal.fire({ icon:'error', title:'Error', text: err.message });
        } finally {
    ...
    window.deleteItem = async function (id) {
        const ask = await Swal.fire({
            icon: 'warning', title: 'Delete Product Family?',
        ...
        try {
            ...
            const res = await fetch("{{ route('erp.product-families.destroy', ':id') }}".replace(':id', id), { method: 'POST', body: fd });
            if (!res.ok) throw new Error('Delete failed');
            DT.ajax.reload(null, false);
            toast('Product Family deleted', 'success');
        } catch (err) {
            Swal.fire({ icon:'error', title:'Error', text: err.message });
        }
    };
