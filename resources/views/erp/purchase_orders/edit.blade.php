...
<div class="container-xxl flex-grow-1 container-p-y">
  ...
  <div class="d-flex align-items-center justify-content-between mb-4">
    <div>
      <h4 class="mb-1 fw-bold text-dark"><i class="bx bx-edit text-primary me-2"></i>Edit Purchase Order</h4>
      <div class="text-muted small">PO No: <span class="badge bg-label-primary fs-7">{{ $purchaseOrder->po_no }}</span> | Reference RF: <span class="fw-semibold">{{ $requestForm->rf_no }}</span></div>
    </div>
  ...
  <form action="{{ route('erp.purchase-orders.update', $purchaseOrder) }}" method="POST" enctype="multipart/form-data" class="card shadow-sm border-0 rounded-4 overflow-hidden" id="poEditForm">
    @csrf
    @method('PUT')
    ...
    <div class="card-header bg-primary bg-opacity-10 py-3 border-bottom">
      <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
        <div>
          <span class="text-uppercase fw-bold text-primary small">Purchase Order No</span>
          <h4 class="mb-0 fw-extrabold text-primary">{{ $purchaseOrder->po_no }}</h4>
        </div>
        <div class="d-flex align-items-center gap-2">
          <span class="badge bg-label-info px-3 py-2 fs-7">{{ $purchaseOrder->status }}</span>
      ...
      <ul class="nav nav-tabs nav-fill po-nav-tabs border-0" id="poTab" role="tablist">
        ...
        <li class="nav-item" role="presentation">
          <button class="nav-link py-3" id="tab-items-btn" data-bs-toggle="tab" data-bs-target="#tab-items" type="button" role="tab">
            <i class="bx bx-package me-2 fs-5"></i>3. PO Items & Pricing <span class="badge bg-primary rounded-pill ms-1">{{ count($purchaseOrder->items) }}</span>
          </button>
        </li>
        <li class="nav-item" role="presentation">
    ...
    <div class="card-body p-4 tab-content" id="poTabContent">
      ...
      <div class="tab-pane fade show active" id="tab-supplier" role="tabpanel">
        ...
        <div class="row g-3">
          <div class="col-md-6">
            ...
            <input type="text" class="form-control bg-light rounded-3" value="{{ $purchaseOrder->po_no }}" readonly>
          </div>

          <div class="col-md-6">
            <label class="form-label fw-semibold">PO Date <span class="text-danger">*</span></label>
            <input type="date" name="date" class="form-control rounded-3 @error('date') is-invalid @enderror" value="{{ old('date', $purchaseOrder->date ? \Carbon\Carbon::parse($purchaseOrder->date)->format('Y-m-d') : now()->format('Y-m-d')) }}" required>
          ...
          <div class="col-md-6">
            ...
            <select name="contact_person" id="contact_person" class="form-select rounded-3 @error('contact_person') is-invalid @enderror">
              ...
              @if($purchaseOrder->contact_person)
                <option value="{{ $purchaseOrder->contact_person }}" selected>{{ $purchaseOrder->contact_person }}</option>
              @endif
            </select>
          ...
          <div class="col-md-6">
            ...
            <input type="date" name="eta" class="form-control rounded-3" value="{{ old('eta', $purchaseOrder->eta ? \Carbon\Carbon::parse($purchaseOrder->eta)->format('Y-m-d') : '') }}">
          </div>

          <div class="col-md-6">
            <label class="form-label fw-semibold">Supplier Address</label>
            <textarea name="address" class="form-control bg-light rounded-3" rows="3" readonly placeholder="Auto filled upon selecting supplier">{{ old('address', $purchaseOrder->address) }}</textarea>
          </div>

          <div class="col-md-6">
            <label class="form-label fw-semibold">Supplier Bank Account</label>
            <input type="text" name="bank_account" class="form-control bg-light rounded-3 mb-2" value="{{ old('bank_account', $purchaseOrder->bank_account) }}" placeholder="Auto filled upon selecting supplier" readonly>
            <label class="form-label fw-semibold mt-2">PO Description / Header Note</label>
            <textarea name="description" class="form-control rounded-3" rows="2" placeholder="General description or note for this PO...">{{ old('description', $purchaseOrder->description) }}</textarea>
          </div>
        </div>
      </div>
      ...
      <div class="tab-pane fade" id="tab-delivery" role="tabpanel">
        ...
        <div class="row g-3">
            ...
            @else
              <div class="input-group">
                ...
                <input type="text" class="form-control rounded-end-3" placeholder="e.g. IT, ATK, Dapur..." 
                  value="{{ str_replace('INTERNAL - ', '', old('project', $purchaseOrder->project ?: '')) }}"
          ...
          <div class="col-md-12">
            ...
            <input type="text" name="transfer_to" class="form-control bg-light rounded-3" value="{{ old('transfer_to', $purchaseOrder->transfer_to) }}" placeholder="Auto filled from Supplier Bank Account" readonly>
          </div>
        </div>
      </div>
      ...
      <div class="tab-pane fade" id="tab-items" role="tabpanel">
        <div class="d-flex align-items-center justify-content-between mb-3">
          <h6 class="fw-bold mb-0 text-primary"><i class="bx bx-package me-1"></i>Items Included in Purchase Order</h6>
          ...
          <table class="table table-hover align-middle mb-0" id="itemsTable">
            ...
            <tbody>
              @foreach($purchaseOrder->items as $idx => $poItem)
                ...
                <tr data-row="{{ $idx }}">
                  <td>
                    <input type="hidden" name="items[{{ $idx }}][request_form_item_id]" value="{{ $poItem->request_form_item_id }}">
        ...
        <div class="row g-3">
          <div class="col-md-6">
            ...
            <div class="paste-dropzone" data-target="#po_attachments_edit_input">
              <input type="file" name="attachments[]" id="po_attachments_edit_input" class="d-none paste-file-input" accept=".jpg,.jpeg,.png,.pdf" multiple>
          ...
          <div class="col-md-6">
            ...
            <textarea name="other_instructions" class="form-control rounded-3" rows="4" placeholder="Special delivery instructions, packaging requirements, or additional terms...">{{ old('other_instructions', $purchaseOrder->other_instructions) }}</textarea>
          </div>
        </div>
      </div>
