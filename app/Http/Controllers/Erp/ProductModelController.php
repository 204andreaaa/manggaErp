...
class ProductModelController extends Controller
{
    public function index()
    {
        ...
        return view('erp.product_models.index');
    }

    public function datatable(Request $r)
    {
        abort_unless(auth()->user()->hasPermission('products.view'), 403);
        $columns = ['id', 'model_name', 'description', 'updated_at'];
        ...
        $rows = $base->orderBy($orderCol, $orderDir)
            ...
            ->map(function ($c, $i) use ($start) {
                $editBtn = auth()->user()->hasPermission('products.update')
                    ? '<button class="btn btn-sm btn-warning text-white me-1" onclick="openEdit('.$c->id.',\''.addslashes(e($c->model_name)).'\',\''.addslashes(e($c->description ?? '')).'\')"><i class="bx bx-edit-alt"></i></button>'
                    : '';
    ...
    public function store(Request $r)
    {
        ...
        $data = $r->validate([
            'model_name'  => ['required', 'max:150', Rule::unique('product_models', 'model_name')],
            'description' => ['nullable', 'max:255'],
        ]);

        ProductModel::create($data);

        return response()->json(['success' => 'Product Model created successfully']);
    }
    ...
    public function update(Request $r, $id)
    {
        ...
        $data = $r->validate([
            'model_name'  => ['required', 'max:150', Rule::unique('product_models', 'model_name')->ignore($model->id)],
            'description' => ['nullable', 'max:255'],
        ]);

        $model->update($data);

        return response()->json(['success' => 'Product Model updated successfully']);
    }
    ...
    public function destroy($id)
    {
        ...
        return response()->json(['success' => 'Product Model deleted successfully']);
    }
}
