...
class BrandController extends Controller
{
    public function index()
    {
        ...
        return view('erp.brands.index');
    }

    public function datatable(Request $r)
    {
        abort_unless(auth()->user()->hasPermission('products.view'), 403);
        $columns = ['id', 'brand_name', 'description', 'updated_at'];
        ...
        $rows = $base->orderBy($orderCol, $orderDir)
            ...
            ->map(function ($c, $i) use ($start) {
                $editBtn = auth()->user()->hasPermission('products.update')
                    ? '<button class="btn btn-sm btn-warning text-white me-1" onclick="openEdit('.$c->id.',\''.addslashes(e($c->brand_name)).'\',\''.addslashes(e($c->description ?? '')).'\')"><i class="bx bx-edit-alt"></i></button>'
                    : '';
    ...
    public function store(Request $r)
    {
        ...
        $data = $r->validate([
            'brand_name'  => ['required', 'max:150', Rule::unique('brands', 'brand_name')],
    ...
    public function update(Request $r, $id)
    {
        ...
        $data = $r->validate([
            'brand_name'  => ['required', 'max:150', Rule::unique('brands', 'brand_name')->ignore($brand->id)],
            'description' => ['nullable', 'max:255'],
        ]);
